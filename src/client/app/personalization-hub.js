function ensureHub() {
  ...
  modal.innerHTML = `
    <div class="w-full max-w-md overflow-hidden bg-shinBg border-4 border-warmBrown rounded-[2rem] shadow-[8px_8px_0_rgba(92,64,51,.28)]">
      <div class="bg-pastelBlue border-b-4 border-warmBrown px-5 py-4 flex items-center justify-between">
        <div>
          <h2 class="text-xl font-bold text-warmBrown">個人化</h2>
          <p class="text-[11px] text-warmBrown/60 font-bold mt-1">選擇要調整的背景區域</p>
        </div>
        <button id="personalization-hub-close" type="button" class="w-9 h-9 rounded-full bg-white border-2 border-warmBrown text-warmBrown"><i class="fas fa-times"></i></button>
      </div>
