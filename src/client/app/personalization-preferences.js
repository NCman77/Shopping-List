...
export function normalizePersonalization(value = {}) {
  ...
  return {
    backgroundFileId: String(source.backgroundFileId ?? '').trim(),
    backgroundFileName: String(source.backgroundFileName ?? '').trim(),
    backgroundMimeType: String(source.backgroundMimeType ?? '').trim().toLowerCase(),
